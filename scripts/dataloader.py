...
class DataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            att_feat = np.load(os.path.join(self.input_att_dir, str(self.info['images'][ix]['file_path'].split('.')[0]) + '.npy'))
            #att_feat = np.load(os.path.join(self.input_att_dir, str(self.info['images'][ix]['id']) + '.npy')
            if att_feat.ndim == 3:
                att_feat = att_feat.reshape(-1, att_feat.shape[-1])
            #return (np.load(os.path.join(self.input_fc_dir, str(self.info['images'][ix]['id']) + '.npy')), att_feat, ix)
            return (np.load(os.path.join(self.input_att_dir, str(self.info['images'][ix]['file_path'].split('.')[0]) + '.npy')).mean(0), att_feat, ix)
        else:
            return (np.load(os.path.join(self.input_att_dir, str(self.info['images'][ix]['file_path'].split('.')[0]) + '.npy')).mean(0), np.zeros((1,1)), ix)
